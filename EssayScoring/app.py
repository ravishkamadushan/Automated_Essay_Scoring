...
def preprocess_text(text):
    ...
    processed_words = [word for word in text.split() if word not in stopwords_list]  # Remove stopwords
    return ' '.join(processed_words), len(processed_words)

# Streamlit app
st.title('Automated Essay Scoring')

# Text input
user_input = st.text_area('Enter your essay here:', height=200)

if st.button('Predict Score'):
    if user_input:
        processed_input, word_count = preprocess_text(user_input)
        if word_count < 3:
            st.success('Predicted Score: 0 (Input is considered meaningless)')
        else:
        ...
        st.error('Please enter some text to score.')
